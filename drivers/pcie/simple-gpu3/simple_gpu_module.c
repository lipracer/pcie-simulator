...
static int sgpu_open(struct inode *inode, struct file *fp) {
  ...
  sdev = container_of(inode->i_cdev, sgpu_dev, cdev);
...
static int sgpu_init_bar(struct pci_dev *pdev, struct sgpu_bar *bar,
  ...
  dev_err(&(pdev->dev),
          "bar %d start address start:0x%llux end:0x%llux len:0x%llux\n", index,
...
static void init_irq_entries(struct sgpu_dev *sdev) {
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    err = request_irq(sdev->entries[i].vector, all_handlers[i],
...
static int sgpu_module_init(void) {
  int err;
